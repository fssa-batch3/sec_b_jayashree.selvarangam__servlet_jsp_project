...
<title>update product with prices</title>
<style>
/*     body {
...
<form action="update_with_prices?productId=<%= product.getProduct_id()  %>" method="post">
    ...
    <%if( price.getSize().getSizeId() == 1){ %>
     <div>
        <label for="largePrice">Large Price:</label>
        <input type="number"  name="largePrice" value="<%= price.getPrice() %>" required min="10" max="1000" step="0.01">
    </div>
    <%} %>
     
      <%  if( price.getSize().getSizeId() == 2){ %>
      <div>
        <label for="mediumPrice">Medium Price:</label>
        <input type="number"  name="mediumPrice" value="<%= price.getPrice() %>" required min="10" max="1000" step="0.01">
    </div>
    <%} %>
    
    <%  if( price.getSize().getSizeId() == 3){ %>
      <div>
        <label for="smallPrice">Small Price:</label>
        <input type="number"  name="smallPrice" value="<%= price.getPrice() %>" required min="10" max="1000" step="0.01">
    </div>
    <%} %>
    <%
    }
